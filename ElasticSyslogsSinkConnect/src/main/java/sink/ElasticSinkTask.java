...
/**
 * ElasticSinkTask is a Task that receives raw and preprocessed syslog data
...
public class ElasticSinkTask extends SinkTask {
    /**
     * An object responsible for storing information about the columns in csv schema (raw syslog schema)
    ...
    /**
     * An object keeping information about the JSON schema (raw syslog records),
    ...
    /**
     * An object keeping information about the JSON schema (preprocessed syslog records),
     * that will be used for storing records in Elastic. It loads a JSON file, which has information
     * about the mapping between the input csv schema and the output JSON schema for Elastic. It converts
     * a csv record to an Elastic object with the proper schema. It also contains some helper functions.
     */
    ...
    /**
     * A boolean variable, which defines if the raw incoming syslogs, will be also inserted
     * to Elastic. Default: False
     */
    private boolean SINK_RAW_SYSLOG = false;
    ...
    public void start(Map<String, String> props) {
        ...
        try {
            SINK_RAW_SYSLOG = Boolean.parseBoolean(props.get("connector.sink.syslog.raw"));
        } catch (Exception e) {
            logger.warn("Cannot read property connector.sink.syslog.raw from file elastic-sink.properties.");
            logger.warn("Using default value (false). Raw syslog data will not be ingested in Elastic.");

            SINK_RAW_SYSLOG = false;
        }

        logger.info("Raw Netflow Data will be ignored: " + !SINK_RAW_SYSLOG);
    ...
    public void put(Collection<SinkRecord> sinkRecords) {
        ...
        for (SinkRecord record : sinkRecords) {
            ...
            if (record.topic().equals(config.get("kafka.topic.sink.syslog.raw"))) {
                counterRaw++;

                if (SINK_RAW_SYSLOG) {
                    // Raw syslog data. Parse and convert to Elastic JSON (if enabled)
                    ...
                    try {
                        ...
                        String elasticIndex = config.get("elastic.index.syslog.raw");
            ...
            } else if (record.topic().equals(config.get("kafka.topic.sink.syslog.preprocessed"))) {
                counterPreprocessed++;
                // Preprocessed syslog data. Parse and convert to Elastic JSON.
                ...
                try {
                    ...
                    // Insert them to Elastic index
                    String elasticIndex = config.get("elastic.index.syslog.preprocessed");
        ...
        logger.info("Retrieved " + sinkRecords.size() + " messages (" + counterRaw + " raw syslogs, " + counterPreprocessed + " prepprocessed syslogs)!");
    }
    ...
    public void stop() {
        ...
        logger.info("Stopping syslog sink task.");
    }
